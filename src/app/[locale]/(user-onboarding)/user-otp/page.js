...
export default function UserOTPVerification() {
  ...
  // Get user ID from localStorage on mount (sync)
  useEffect(() => {
    const id = getUserId();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const resp = await verifyUserOTP(userId, code);

      if (resp?.statusCode === 200) {
        setSuccess("OTP verified successfully! Redirecting...");

        // Try to get user detail from OTP response; if missing, fetch via API
        let detail = resp?.data?.userDetails || null;
        if (!detail) {
          detail = await fetchUserProfileFromAPI();
        }

        // Save session (sets token, caches profile, flips onboarding flag, emits header event)
        saveUserSession({
          userId,
          token: resp?.data?.token,
          userDetails: detail || undefined,
        });

        const complete = isUserProfileComplete(detail);
        router.replace(complete ? "/profile" : "/user-details");
      } else {
        setError(resp?.message || "OTP verification failed");
      }
    } catch (err) {
  ...
  const handleResendOTP = async () => {
    ...
    try {
      const data = await resendUserOTP(userId);
